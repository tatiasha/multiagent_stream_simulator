package itmo.escience.streamsimulator.algorithms.signal_distr_ga.heuristic

import java.util.Random

import itmo.escience.streamsimulator.entities.Node
import itmo.escience.streamsimulator.utilities.{ExpProps, Misc}
import itmo.escience.streamsimulator.{OperatorId, ResourceId}
import itmo.escience.streamsimulator.algorithms.signal_distr_ga.{SDGene, SDChromosome, SDSolution}
...
object SDHeuristicAlgorithm {
  ...
  def estimatePlacement(nodeId: ResourceId, op: OperatorId, map: util.HashMap[ResourceId, List[OperatorId]], stats: AppStats): Double = {
    ...
    for (pOp <- placedOps :+ op) {
      val opStat = opStats.get(pOp).get
  ...
  def availableNodes(stats: AppStats, sched: util.HashMap[ResourceId, List[OperatorId]]): List[ResourceId] = {
    ...
    for (key <- sched.keySet) {
      ...
      for (pOp <- nodeOps) {
        val opStat = stats.opStats.get(pOp).get
        for (i <- 0 until timeBound) {
          val st = opStat(i)
          val thr = st.inputThroughput.toDouble
          mergedSignal(i) += thr
      ...
      val avg = mergedSignal.sum / mergedSignal.size

...
import itmo.escience.streamsimulator.{ContainerId, ResourceId, SlotId}
...
class Environment(var resources: List[Resource], var networks: List[Network])
  ...
  private[streamsimulator] var resMap: util.HashMap[ResourceId, Resource] = new util.HashMap[ResourceId, Resource]()
  private[streamsimulator] var slotMap: util.HashMap[SlotId, Slot] = new util.HashMap[SlotId, Slot]()
  private[streamsimulator] var containerIds: List[ResourceId] = List[ResourceId]()
